import { Component, inject, OnInit, ChangeDetectorRef, Optional, Inject } from '@angular/core';
...
@Component({
  ...
  template: `
    <div class="dialog-container">
      ...
      <mat-dialog-content>
        @if (!isEditMode) {
          <p class="dialog-subtitle">Define a new role to group specific platform permissions.</p>
        }
        @if (isEditMode) {
        ...
        <form [formGroup]="form" class="role-form">
          <div class="form-group">
            ...
            <input type="text" formControlName="name" class="premium-input" placeholder="e.g. Support Manager">
          </div>
    
          <div class="form-group">
            <label>Role Code <span class="required">*</span></label>
            <input type="text" formControlName="code" class="premium-input" placeholder="e.g. SUPPORT_MANAGER">
            <span class="hint-text">Unique identifier for this role (uppercase, underscores).</span>
          </div>
    
          <div class="form-group">
            <label>Description</label>
            <textarea formControlName="description" class="premium-input" rows="3" placeholder="Briefly describe the purpose of this role..."></textarea>
          </div>
        </form>
        ...
        @if (!isEditMode && !isLoadingPermissions && allPermissions.length === 0) {
        ...
        @if (!isEditMode && !isLoadingPermissions && permissionGroups.length > 0) {
...
export class RbacCreateRoleDialogComponent implements OnInit {
  ...
  save() {
    ...
    if (this.isEditMode) {
      this.rbacService.updateRole(this.data.role.id, {
