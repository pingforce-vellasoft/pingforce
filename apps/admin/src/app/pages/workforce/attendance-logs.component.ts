...
@Component({
  ...
  standalone: true,
  imports: [
    CommonModule, RouterModule, MatTableModule, MatCardModule, MatIconModule, 
    MatMenuModule, MatButtonModule, MatDividerModule, MatDialogModule,
    MatPaginatorModule, MatSortModule, MatInputModule, MatRippleModule
  ],
  template: `
      ...
      <div class="table-wrapper">
        <table mat-table [dataSource]="dataSource" matSort class="premium-table">
          ...
          <ng-container matColumnDef="employee">
            <th mat-header-cell *matHeaderCellDef mat-sort-header> <mat-icon class="header-icon">person</mat-icon> Employee </th>
            <td mat-cell *matCellDef="let element">
              <div class="employee-cell">
                <div class="avatar">{{getInitials(element.employee)}}</div>
                <div class="emp-details">
                  <a [routerLink]="['/dashboard/workforce/employee', element.employee.id]" class="emp-name">{{getName(element)}}</a>
                  <span class="emp-role">{{element.employee?.department?.name || 'Staff'}}</span>
                </div>
              </div>
            </td>
          </ng-container>
          ...
          <ng-container matColumnDef="shortfalls">
            <th mat-header-cell *matHeaderCellDef> <mat-icon class="header-icon">trending_down</mat-icon> Shortfalls </th>
            <td mat-cell *matCellDef="let element">
              <span class="shortfall-badge" [class.has-shortfalls]="(element.employee.shortfallDays || 0) > 0">
          ...
          <ng-container matColumnDef="leaves">
            <th mat-header-cell *matHeaderCellDef> <mat-icon class="header-icon">event_available</mat-icon> Leaves </th>
            <td mat-cell *matCellDef="let element">
              <div class="leave-balance">
                <span class="leave-count">{{ element.employee.leaveBalance || 0 }}</span>
          ...
          <ng-container matColumnDef="checkIn">
            <th mat-header-cell *matHeaderCellDef mat-sort-header> <mat-icon class="header-icon">login</mat-icon> Check In </th>
            <td mat-cell *matCellDef="let element">
              <div class="cell-content date-cell">
                <div class="time-main">{{element.checkInTime | date:'shortTime'}}</div>
                <div class="time-sub">{{element.checkInTime | date:'mediumDate'}}</div>
              </div>
            </td>
          </ng-container>
    
          <ng-container matColumnDef="checkOut">
            <th mat-header-cell *matHeaderCellDef mat-sort-header> <mat-icon class="header-icon">logout</mat-icon> Last Check Out </th>
            <td mat-cell *matCellDef="let element">
              <div class="cell-content date-cell">
                @if (element.checkOutTime) {
                  <div class="time-main">{{element.checkOutTime | date:'shortTime'}}</div>
                  <div class="time-sub">{{element.checkOutTime | date:'mediumDate'}}</div>
                } @else {
          ...
          <ng-container matColumnDef="hours">
            <th mat-header-cell *matHeaderCellDef> <mat-icon class="header-icon">timer</mat-icon> Duration </th>
            <td mat-cell *matCellDef="let element">
              <div style="display: flex; flex-direction: column; align-items: flex-start; gap: 4px;">
                <div class="duration-container" [ngClass]="getDurationClass(element)">
          ...
          <div class="page-size-selector">
            ...
            <div class="select-wrapper">
              ...
              <mat-menu #sizeMenu="matMenu" class="dark-menu">
                <button mat-menu-item (click)="setPageSize(5)" [class.active]="pageSize === 5">5 rows</button>
                <button mat-menu-item (click)="setPageSize(10)" [class.active]="pageSize === 10">10 rows</button>
                <button mat-menu-item (click)="setPageSize(25)" [class.active]="pageSize === 25">25 rows</button>
                <button mat-menu-item (click)="setPageSize(100)" [class.active]="pageSize === 100">100 rows</button>
              </mat-menu>
            </div>
          </div>
          <mat-paginator [length]="totalRecords" [pageSize]="pageSize" hidePageSize="true" class="premium-paginator"></mat-paginator>
        </div>
      </div>
    </div>
    `,
  styles: [`
    ...
    .emp-name { font-weight: 600; font-size: 14px; color: #f8fafc; text-decoration: none; }
    .emp-name:hover { color: #8b5cf6; text-decoration: underline; }
    .emp-role { font-size: 12px; color: #94a3b8; }
    ...
    .time-main { font-weight: 600; font-size: 14px; color: #f8fafc; }
    .time-sub { font-size: 12px; color: #94a3b8; }
    .action-cell { text-align: right; width: 60px; }
    .action-btn { color: #94a3b8; }
    .action-btn:hover { color: #f8fafc; background: rgba(255,255,255,0.05); }
    ...
    .empty-state h3 { margin: 0; color: #cbd5e1; font-weight: 500; font-size: 18px; }
...
export class AttendanceLogsComponent implements OnInit, AfterViewInit {
  ...
  displayedColumns: string[] = ['employee', 'shortfalls', 'leaves', 'checkIn', 'checkOut', 'hours', 'actions'];
  dataSource: any[] = [];
  ...
  loadData() {
    ...
    this.workforceService.getAttendanceLogs(page, limit, this.searchTerm, sortField, sortDir).subscribe({
      next: (data) => {
        if (data.data && data.data.length > 0) {
          ...
          this.dataSource = [
            {
              employee: { id: 'emp-1', user: { profile: { firstName: 'Sarah', lastName: 'Connor' } }, department: { name: 'Field Operations' }, shortfallDays: 0, leaveBalance: 12 },
              attendanceMethod: 'BIOMETRIC',
              checkInTime: new Date(Date.now() - 8 * 60 * 60 * 1000),
              checkOutTime: null
            },
            {
              employee: { id: 'emp-2', user: { profile: { firstName: 'John', lastName: 'Wick' } }, department: { name: 'Security' }, shortfallDays: 2, leaveBalance: 4 },
            ...
            {
              employee: { id: 'emp-3', user: { profile: { firstName: 'Ellen', lastName: 'Ripley' } }, department: { name: 'Logistics' }, shortfallDays: 1, leaveBalance: 8 },
              attendanceMethod: 'MANUAL',
              checkInTime: new Date(Date.now() - 56 * 60 * 60 * 1000),
              checkOutTime: new Date(Date.now() - 48 * 60 * 60 * 1000)
            },
            {
              employee: { id: 'emp-4', user: { profile: { firstName: 'Peter', lastName: 'Parker' } }, department: { name: 'Photography' }, shortfallDays: 4, leaveBalance: 0 },
    ...
    if (log.checkInTime && log.checkOutTime) {
      ...
      for (const session of log.sessions) {
        if (session.checkInTime && session.checkOutTime) {
          totalMs += (new Date(session.checkOutTime).getTime() - new Date(session.checkInTime).getTime());
        }
      }
    }
  ...
  getDurationClass(log: any): string {
    ...
    const hasCheckOut = log.checkOutTime || (log.sessions && log.sessions.length > 0 && log.sessions[0].punchOut);
  ...
  manualCheckout(log: any) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        this.workforceService.manualCheckout(sessionId, result.checkoutTime, result.reason).subscribe({
          next: () => {
