  ...
  template: `
    ...
    <mat-card class="table-container">
      <table mat-table [dataSource]="dataSource">
        ...
        <ng-container matColumnDef="priority">
          ...
          <td mat-cell *matCellDef="let element">
            <span
              class="status-badge"
              [ngClass]="element.priority.toLowerCase()"
            >
              {{ element.priority }}
            </span>
          </td>
      ...
      @if (dataSource.length === 0) {
        <div class="empty-state">No active support tickets found!</div>
      }
    </mat-card>
  `,
  styles: [
  ...
  escalate(id: string) {
    const dialogRef = this.dialog.open(ConfirmDialogComponent, {
      ...
      data: {
        ...
        message:
          'Are you sure you want to trigger a CQRS escalation for this ticket?',
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
        this.crmService.escalateFault(id).subscribe({
          next: () => {
            this.snackBar.open(
              'Ticket escalated via CQRS Command Bus!',
              'Close',
              { duration: 4000 },
            );
            this.loadData();
