  ...
  template: `
    <div class="premium-dialog">
      ...
      <mat-dialog-content class="dialog-content">
        <div class="module-card" [class.active]="data.tenant.isAttendanceEnabled" (click)="data.tenant.isAttendanceEnabled = !data.tenant.isAttendanceEnabled">
          <div class="module-info">
            <mat-icon class="module-icon">location_on</mat-icon>
            <div class="module-text">
              <span class="module-title">GPS & Biometric Attendance</span>
              <span class="module-desc">Enable location-based check-ins and facial recognition</span>
            </div>
          </div>
          <div class="custom-toggle" [class.active]="data.tenant.isAttendanceEnabled">
        ...
        @if (data.tenant.isAttendanceEnabled) {
          <div class="form-container">
            <div class="custom-field mt-16">
              ...
              <input type="number" [(ngModel)]="data.tenant.maxFieldStaff" placeholder="Unlimited if empty">
      ...
      <mat-dialog-actions align="end" class="dialog-actions">
        <button mat-button mat-dialog-close class="custom-btn btn-cancel">Cancel</button>
        <button mat-button color="primary" [mat-dialog-close]="data.tenant" class="custom-btn btn-save">Save Changes</button>
      </mat-dialog-actions>
    </div>
    `,
  styles: [`
    ...
    .btn-save:hover { background: #4f46e5 !important; }
    .btn-cancel { color: var(--text-secondary) !important; }
    .btn-cancel:hover { color: var(--text-primary) !important; background: rgba(255, 255, 255, 0.05) !important; }
  ...
  template: `
    <div class="confirm-dialog-container">
      ...
      <mat-dialog-actions align="end" class="dialog-actions">
        ...
        <button mat-flat-button [color]="data.isDanger ? 'warn' : 'primary'" [mat-dialog-close]="true" class="confirm-btn" [class.danger-btn]="data.isDanger">
          {{ data.confirmText }}
        </button>
      </mat-dialog-actions>
    </div>
...
export class ConfirmActionDialogComponent {
  constructor(@Inject(MAT_DIALOG_DATA) public data: { title: string, message: string, subMessage?: string, confirmText: string, isDanger: boolean, icon?: string }) {}
}

@Component({
  selector: 'app-platform-tenants',
  standalone: true,
  imports: [MatCardModule, MatTableModule, MatButtonModule, MatIconModule, MatDialogModule, MatSnackBarModule, MatSlideToggleModule, FormsModule, RouterModule, MatMenuModule, NgClass],
  template: `
    <div class="page-container">
      <div class="page-header">
        <div class="header-content">
          <h2 class="page-title">Tenant Management</h2>
          <p class="page-subtitle">Manage platform tenants, their subscriptions, and module provisioning.</p>
        </div>
        <div class="header-actions">
          <button mat-raised-button color="primary" class="action-btn" routerLink="/dashboard/platform/tenants/create">
        ...
        <mat-card-content class="no-padding">
          <table mat-table [dataSource]="tenants" class="full-width-table">
              ...
              <td mat-cell *matCellDef="let tenant">
                <div class="tenant-name-cell">
                  ...
                  <div class="tenant-details">
                    <a [routerLink]="['/dashboard/platform/tenants', tenant.id]" class="tenant-name-link">
            ...
            <ng-container matColumnDef="contact">
              ...
              <td mat-cell *matCellDef="let tenant">
                <div class="cell-stack">
                  <span class="cell-primary">{{tenant.contactEmail || 'N/A'}}</span>
                  <span class="cell-secondary">{{tenant.contactPhone || 'N/A'}}</span>
                </div>
              </td>
            </ng-container>
            ...
            <ng-container matColumnDef="subscription">
              ...
              <td mat-cell *matCellDef="let tenant">
                <div class="cell-stack">
                  <span class="cell-primary">{{tenant.subscriptionPlan || 'N/A'}}</span>
                  <span class="status-badge" [ngClass]="tenant.subscriptionStatus?.toLowerCase() || 'unknown'">{{tenant.subscriptionStatus || 'N/A'}}</span>
                </div>
              </td>
            </ng-container>
            ...
            <ng-container matColumnDef="actions">
              ...
              <td mat-cell *matCellDef="let tenant">
                ...
                <mat-menu #menu="matMenu" class="premium-menu" yPosition="below" xPosition="before">
                  ...
                  <button mat-menu-item [routerLink]="['/dashboard/platform/tenants', tenant.id]">
                    <div class="menu-item-content">
                  ...
                  <button mat-menu-item (click)="toggleTenantStatus(tenant)">
                    <div class="menu-item-content">
                      <mat-icon [color]="tenant.subscriptionStatus === 'SUSPENDED' ? 'primary' : 'warn'">
                        {{ tenant.subscriptionStatus === 'SUSPENDED' ? 'check_circle' : 'block' }}
                      </mat-icon>
                      <span>{{ tenant.subscriptionStatus === 'SUSPENDED' ? 'Enable Tenant' : 'Disable Tenant' }}</span>
                    </div>
                  </button>
...
export class TenantsComponent implements OnInit {
  ...
  displayedColumns: string[] = ['name', 'contact', 'location', 'subscription', 'activeModules', 'actions'];
  tenants: any[] = [];
  ...
  loadTenants() {
    this.http.get<any[]>('/api/v1/tenants').subscribe({
      ...
      error: () => this.snackBar.open('Failed to load tenants', 'Close', { duration: 3000 })
    });
  }
    ...
    if (!isEnabled) {
      ...
      const dialogRef = this.dialog.open(ConfirmActionDialogComponent, {
        ...
        data: {
          ...
          subMessage: 'Field staff will no longer be able to punch in/out. This action will not delete historical data, but active tracking will be suspended immediately.',
  ...
  private executeToggle(tenant: any, isEnabled: boolean) {
    ...
    this.http.patch(`/api/v1/tenants/${tenant.id}/provisioning`, {
      isAttendanceEnabled: isEnabled,
      maxFieldStaff: tenant.maxFieldStaff
    }).subscribe({
      next: () => {
        this.snackBar.open(`Attendance module ${isEnabled ? 'enabled' : 'disabled'} for ${tenant.name}`, 'Close', { duration: 3000 });
      },
      error: () => {
        tenant.isAttendanceEnabled = !isEnabled; // Revert
        this.snackBar.open('Failed to update tenant', 'Close', { duration: 3000 });
      }
    });
  }
    ...
    if (isSuspending) {
      const dialogRef = this.dialog.open(ConfirmActionDialogComponent, {
        ...
        data: {
          ...
          subMessage: 'Their users will immediately lose access to the platform. No data will be deleted.',
  ...
  private executeTenantStatusToggle(tenant: any, newStatus: string) {
    this.http.patch(`/api/v1/tenants/${tenant.id}`, { subscriptionStatus: newStatus, status: newStatus }).subscribe({
      next: () => {
        this.snackBar.open(`Tenant ${newStatus === 'ACTIVE' ? 'enabled' : 'disabled'} successfully`, 'Close', { duration: 3000 });
        this.loadTenants();
      },
  ...
  deleteTenant(tenant: any) {
    const dialogRef = this.dialog.open(ConfirmActionDialogComponent, {
      ...
      data: {
        ...
        subMessage: 'This action will soft-delete the tenant and they will be removed from your active roster. This cannot be undone from the UI.',
