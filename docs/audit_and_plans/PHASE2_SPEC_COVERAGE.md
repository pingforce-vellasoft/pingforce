...
| **Deviation:** spec §3 says refresh tokens must be _opaque random values, never JWTs_ — current implementation signs a JWT and stores its `jti`. Functionally rotated+hashed-equivalent, but non-conformant. Refactor in Phase 3. |
