...
import { CrmSupportService, Lead } from '../../core/services/crm-support.service';
...
@Component({
      ...
      <table mat-table [dataSource]="dataSource">
        ...
        <ng-container matColumnDef="contact">
          ...
          <td mat-cell *matCellDef="let element">
            ...
            <div class="text-small text-muted">{{element.phone || 'N/A'}}</div>
          </td>
        </ng-container>
        ...
        <ng-container matColumnDef="stage">
          ...
          <td mat-cell *matCellDef="let element">
            <span class="status-badge" [ngClass]="element.pipelineStage?.name?.toLowerCase()">
        ...
        <ng-container matColumnDef="actions">
          ...
          <td mat-cell *matCellDef="let element" class="action-cell">
            <button mat-stroked-button color="primary" class="action-btn" (click)="advanceStage(element)">
  ...
  styles: [`
    ...
    .lost { background: #ffebee; color: #d32f2f; }
  `]
})
export class LeadsComponent implements OnInit {
  ...
  displayedColumns: string[] = ['name', 'company', 'contact', 'stage', 'actions'];
  dataSource: Lead[] = [];
  ...
  advanceStage(lead: Lead) {
    ...
    const promptStage = prompt('Enter new stage ID (e.g., Contacted, Qualified):');
    if (promptStage) {
      this.crmService.updateLeadStage(lead.id, promptStage).subscribe({
        next: () => {
          this.snackBar.open('Lead stage advanced successfully', 'Close', { duration: 3000 });
          this.loadData();
        },
        error: (err) => {
