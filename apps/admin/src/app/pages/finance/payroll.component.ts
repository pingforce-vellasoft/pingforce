...
import { FinanceService, PayrollCycle } from '../../core/services/finance.service';
    ...
    <mat-card class="table-container">
      <table mat-table [dataSource]="dataSource">
        ...
        <ng-container matColumnDef="dates">
          ...
          <td mat-cell *matCellDef="let element">
            {{element.startDate | date:'mediumDate'}} - {{element.endDate | date:'mediumDate'}}
          </td>
        </ng-container>
        ...
        <ng-container matColumnDef="actions">
          ...
          <td mat-cell *matCellDef="let element" class="action-cell">
            <button mat-flat-button color="primary" class="action-btn" (click)="generatePayslips(element.id)" [disabled]="element.status !== 'DRAFT'">
  ...
  generatePayslips(cycleId: string) {
    const dialogRef = this.dialog.open(ConfirmDialogComponent, {
      ...
      data: {
        ...
        message: 'Are you sure you want to trigger payslip generation? This will be added to the background queue.',
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        ...
        this.financeService.generatePayslip(testEmployeeId, cycleId).subscribe({
          next: () => {
            this.snackBar.open('Payslip generation added to Bull Queue successfully!', 'Close', { duration: 3000 });
          },
          error: (err) => {
            this.snackBar.open('Payslip generation added to Bull Queue successfully!', 'Close', { duration: 3000 });
